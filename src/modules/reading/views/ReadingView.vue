...
<template>
  <section class="rounded-2xl border border-slate-800 bg-slate-900/60 p-5 sm:p-7">
    ...
    <div
      ...
      @click.self="onCancelFinish"
    >
      <section class="w-full max-w-md rounded-2xl border border-slate-800 bg-slate-900 p-4 shadow-2xl sm:p-6">
        <h2 class="text-lg font-semibold text-white">{{ t('reading.finishStepTitle') }}</h2>
        <p class="mt-1 text-sm text-slate-300">{{ t('reading.finishStepSubtitle') }}</p>

        <label class="mt-4 block text-xs text-slate-300">
          {{ t('reading.endPage') }}
          <input
            v-model="finishEndPage"
            type="number"
            min="0"
            class="mt-1 w-full rounded-xl border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-slate-100 outline-none ring-cyan-400 focus:ring-2"
          />
        ...
        <div class="mt-4 grid grid-cols-2 gap-2">
          ...
          >
            {{ saving ? t('reading.savingSession') : t('reading.confirmFinish') }}
          </button>
        </div>
      </section>
    </div>
